...
def train(model, config: ModelConfig,
    ...
    for epoch_num in trange(config.epochs):
        ...
        for step_num, batch in tqdm(enumerate(train_dl), total=train_steps):
            ...
            u_exact = batch.target
            u_obs = batch.u_obs

            # Forward pass - get ML model prediction of u
            u_ml = model(u_obs)

            # Solve PDE for u, given k and assemble the L2-loss: 0.5 * ||G(u) - u_obs||^{2}_{L2}
            loss = G(u_ml, u_obs)
            total_loss += loss_u_pde.item()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", default="nn", type=str, help="one of [encoder-decoder, cnn]")
    ...
    def solve_pde(u_in, u_obs, f, V, bcs, ndt):
    ...
    solve_pde = functools.partial(solve_pde, f=f, V=V, bcs=bcs, ndt=ndt)
    ...
    with set_working_tape() as tape:
        ...
        F = ReducedFunctional(solve_pde(u_init, u_obs), [Control(u_init), Control(u_obs)])
        G = torch_operator(F)
